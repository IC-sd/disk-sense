...
import { attribute, storageRelationship, findRelatedLocationsAsync } from '../desktop/app-attribution.cjs'
...
describe('application attribution', () => {
  ...
  it('returns bounded existing locations for a known application', async () => {
    const asyncLocations = await findRelatedLocationsAsync(
      'C:/Users/me/AppData/Local/Google/Chrome/User Data/Cache/a',
      ['C:\\']
    )
    expect(asyncLocations.length).toBeLessThanOrEqual(16)
    expect(asyncLocations.every((item: any) => item.path && item.volume)).toBe(true)
  })
